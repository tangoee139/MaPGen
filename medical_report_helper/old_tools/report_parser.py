...
class ResultItem:
    def __init__(self, buwei_notchange = '',zibuwei_notchange = '', desc_notchange = '', norm_buwei='', origin_buwei='', norm_zibuwei='',
    ...
    def get_upword_label(self):
        if self.upword_buwei == "" or self.upword_desc == "":
            # logging.info(f"upword_buwei: {str(self.upword_buwei)}, "
            #              f"upword_zibuwei: {str(self.upword_zibuwei)}, "
            #              f"upword_desc: {str(self.upword_desc)}",
            #              f"origin_sentence: {str(self.origin_sentence)}")
            # return f"有内容为: {self.upword_label}"
            return ""
        else:
            return self.upword_label

    def get_normal_label(self):
        if self.norm_buwei == "" or self.norm_desc == "":
            # logging.info(f"norm_buwei: {str(self.norm_buwei)}, "
            #              f"norm_zibuwei: {str(self.norm_zibuwei)}, "
            #              f"norm_desc: {str(self.norm_desc)}, "
            #              f"origin_sentence: {str(self.origin_sentence)}")
            # return f"有内容为: {self.normal_label}"
            return ""
        else:
    ...
    def post_process(self, res_object: ResultItem) -> ResultItem:
        ...
        if keyword in res_object.origin_buwei:
            ...
            if "内侧" in res_object.origin_buwei:
                ...
                res_object.upword_buwei = ["盘状半月板", "内侧半月板"]
            elif "外侧" in res_object.origin_buwei:
                res_object.origin_buwei = ["盘状半月板", "外侧半月板"]
                res_object.norm_buwei = ["盘状半月板", "外侧半月板"]
                res_object.upword_buwei = ["盘状半月板", "外侧半月板"]
            elif "内外" in res_object.origin_buwei:
                res_object.origin_buwei = ["盘状半月板", "内侧半月板", "外侧半月板"]
                res_object.norm_buwei = ["盘状半月板", "内侧半月板", "外侧半月板"]
                res_object.upword_buwei = ["盘状半月板", "内侧半月板", "外侧半月板"]
            else:
                res_object.origin_buwei = ["盘状半月板","半月板"]
                res_object.norm_buwei = ["盘状半月板","半月板"]
                res_object.upword_buwei = ["盘状半月板","半月板"]
