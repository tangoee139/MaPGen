...
# 配置日志记录器
# logging.basicConfig(filename='../output/output_diagnosis/error.log', level=logging.INFO, filemode='w', format='%(asctime)s - %(levelname)s - %(message)s')
# logging.info(f"*********新的记录*********")
...
class ResultItem:
    def __init__(self, buwei_notchange = '',zibuwei_notchange = '', desc_notchange = '', norm_buwei='', origin_buwei='',
                 norm_zibuwei='',
        ...
        self.desc_notchange = desc_notchange # 为了计算分数保留的没有修改的描述
        ...
        # 程度
        self.origin_level = origin_level
    ...
    def get_upword_label(self):
        if self.upword_label in alone_sentence_list:
            return self.upword_label

        if self.upword_buwei == "" :
            logging.info(f"上位部位为空： {str(self.origin_sentence)}")
            return ""
        elif self.upword_desc == "":
            logging.info(f"上位描述为空： {str(self.origin_sentence)}")
            return ""
        else:
            return self.upword_label

    def get_normal_label(self):
        # if self.normal_label in alone_sentence_list:
        #     obj_res = ResultItem()
        #     return obj_res

        # if self.norm_buwei == "" and self.origin_zibuwei == "":
        #     logging.info(f"norm部位和子部位为空： {str(self.origin_sentence)}")
        #     return ""
        if self.norm_desc == "":
            logging.info(f"norm描述为空： {str(self.origin_sentence)}")
            return ""
        else:
            # res = (f'部位：{self.norm_buwei}；子部位：{self.norm_zibuwei}；描述：{self.norm_desc}；程度：{self.normal_level}'
...
class ReportParser():
    ...
    ### 资源初始化相关函数
    def __init__(self, keywords_xlsx: str = "D:\IT\Python\PythonProject\msiip_medical_report_helper\data\膝关节概念关键词表（新）.xlsx"):
        # keywords_xlsx: 关键词表
        # 其中有三个子表: 部位、子部位、描述
        ...
        self.df_alone_sentence = pd.read_excel(self.keywords_xlsx, sheet_name="单独句")

        self.df_subPart_key = pd.read_excel(self.keywords_xlsx, sheet_name="子部位")

        self.df_diagnosis_desc = pd.read_excel(self.keywords_xlsx, sheet_name="描述关键词表_程度")
        self.df_diagnosis_desc = self.df_diagnosis_desc.dropna(subset=['描述词'])

        self.df_level_key = pd.read_excel(self.keywords_xlsx, sheet_name="描述程度")
        ...
        self.read_level_keywords()
        self.read_alone_sentence_keywords()
        ...
        # 程度关键词
        self.list_level_key = list(self.level_alias_map.keys())
        # 单独句关键词
        self.list_alone_sentence_key = list(self.alone_sentence_alias_map.keys())

        # 初始化顿号统一写法字典
        # 要替换为顿号的字
        self.chars_to_check = ["伴","合并", "及", "并"]
        ...
        self.keywords = r'(为著|同前|较前|结合|对比|详见|建议|必要|随诊|原因|假体|未见|复查)'
    ...
    # 程度读取
    def read_level_keywords(self) -> None:
        self.level_dict = {}
        self.level_alias_map = {}

        self.df_level_key = self.df_level_key[['归一化程度', '程度']]
        self.df_level_key = self.expand_data_restore(self.df_level_key, "程度")

        for _, row in self.df_level_key.iterrows():
            norm_name = row["归一化程度"]
            alias = row["程度"]
            ...
            # key: 子部位名称, value: ZiBuWeiItem
            if norm_name in self.level_dict:
                self.level_dict[norm_name].alias.append(alias)
            else:
                self.level_dict[norm_name] = item

            # key: 程度别名, value: 归一化程度名称
            self.level_alias_map[alias] = norm_name

        # for value in self.level_dict.values():
        #     print(value)

    # 单独句读取
    def read_alone_sentence_keywords(self) -> None:
        self.alone_sentence_dict = {}
        self.alone_sentence_alias_map = {}

        self.df_alone_sentence = self.df_alone_sentence[['单独句', '归一化单独句']]
        self.df_alone_sentence = self.expand_data_restore(self.df_alone_sentence, "单独句")

        global alone_sentence_list
        alone_sentence_list = self.df_alone_sentence['单独句'].tolist()
        # print(alone_sentence_list)


        for _, row in self.df_alone_sentence.iterrows():
            norm_name = row["归一化单独句"]
            alias = row["单独句"]

            item = ZiBuWeiItem(norm_name, [alias])

            if norm_name in self.alone_sentence_dict:
                self.alone_sentence_dict[norm_name].alias.append(alias)
            else:
                self.alone_sentence_dict[norm_name] = item

            # key: 程度别名, value: 归一化程度名称
            self.alone_sentence_alias_map[alias] = norm_name
    ...
    def parse_report(self, report: str) -> list:
        ...
        final_result = [self.obj_split(res) for res in final_result if res != ""]
    ...
    def parse_report_list(self, diagnose_list) -> list:
        ...
        data = [
            {
                'norm_label': self.get_norm_label(item),
    ...
    def get_upword_label(self, res_item):
        ...
        if res_item.buwei_notchange != '' or res_item.desc_notchange != '':
            if res_item.buwei_notchange != '':
                if "盘状半月板" in res_item.upword_buwei:
                    flag_panzhuang = 0
                    upword_label.append("盘状半月板")

                    if isinstance(res_item.upword_desc, list):
                        for desc_item in res_item.upword_desc:
                            upword_label.append(res_item.upword_buwei[1] + res_item.upword_zibuwei + desc_item)
                            if len(res_item.upword_buwei) == 3:
                                upword_label.append(res_item.upword_buwei[2] + res_item.upword_zibuwei + desc_item)
                    else:
                        if len(res_item.origin_buwei) > 1:
                            upword_label.append(res_item.upword_buwei[1] + res_item.upword_zibuwei + res_item.upword_desc)
                            if len(res_item.upword_buwei) == 3:
                                upword_label.append(res_item.upword_buwei[2] + res_item.upword_zibuwei + res_item.upword_desc)

                if flag_panzhuang:
                    if isinstance(res_item.upword_buwei, list):
                        for item in res_item.upword_buwei:
                            if isinstance(res_item.upword_desc, list):
                                flag = 0
                                for desc_item in res_item.upword_desc:
                                    upword_label.append(item + res_item.upword_zibuwei + desc_item)
                            else:
                                upword_label.append(item + res_item.upword_zibuwei + res_item.upword_desc)
            if flag and flag_panzhuang:
                if isinstance(res_item.upword_desc, list):
                    for item in res_item.upword_desc:
                        upword_label.append(res_item.upword_buwei + res_item.upword_zibuwei + item)
        else:
    ...
    # 如果要获得origin_label，也可以从这个函数中获得，现在只是没有返回origin_label,但是在这个函数中已经生成了
    def get_norm_label(self,res_item):
        origin_label = []
        norm_label = []
        flag = 1 # 如果部位中已经对子部分展开过一次，则子部位不用在处理
        flag_panzhuang = 1 # 如果是盘状半月板，则不用在处理
        # 在后处理中，部位和子部位都有可能是列表，也就是分开了，现在将整个label分开，形成一个列表f
        # 先判断部位，子部位，描述是否不为空，也就是部位，子部位，描述是是一个列表，如果是列表，就需要展开
        # 首先是部位，部位中，如果是盘状半月板，需要单独处理，然后根据描述是否是空， 不是空就需要就需要将描述和部位合并为label
        #  原来是子部位展开，主要是处理”骨软骨“相关的子部位，现在不需要将骨软骨改为，骨和软骨，但是需要描述展开，将原来的处理子部位逻辑改为描述
        # 设置flag的目的是，如果在上面的步骤中，已经对子部位进行了展开，就不需要再次展开

        # 删除 ：：如果不是盘状半月板，就得将部位的列表展开，接着检查子部位是否是列表，如果是列表，也需要将子部位展开，如果不是就直接与部位合并
        ...
        if res_item.buwei_notchange != '' or res_item.desc_notchange != '':
            if res_item.buwei_notchange != '':
                if "盘状半月板" in res_item.origin_buwei:
                    ...
                    if isinstance(res_item.origin_desc, list):
                        for desc_item in res_item.origin_desc:
                            origin_label.append(res_item.origin_buwei[1] + res_item.origin_zibuwei + desc_item)
                            norm_label.append(res_item.norm_buwei[1] + res_item.norm_zibuwei + desc_item)
                            if len(res_item.origin_buwei) == 3:
                                origin_label.append(res_item.origin_buwei[2] + res_item.origin_zibuwei + desc_item)
                                norm_label.append(res_item.norm_buwei[2] + res_item.norm_zibuwei + desc_item)
                    else:
                        if len(res_item.origin_buwei) > 1:
                            origin_label.append(res_item.origin_buwei[1] + res_item.origin_zibuwei + res_item.origin_desc)
                            norm_label.append(res_item.norm_buwei[1] + res_item.norm_zibuwei + res_item.norm_desc)
                            if len(res_item.origin_buwei) == 3:
                                origin_label.append(res_item.origin_buwei[2] + res_item.origin_zibuwei + res_item.origin_desc)
                                norm_label.append(res_item.norm_buwei[2] + res_item.norm_zibuwei + res_item.norm_desc)
                ...
                if flag_panzhuang:
                    for item in res_item.norm_buwei:
                        if isinstance(res_item.origin_desc, list):
                            flag = 0
                            for desc_item in res_item.origin_desc:
                                origin_label.append(item + res_item.origin_zibuwei + desc_item)
                                norm_label.append(item + res_item.norm_zibuwei + desc_item)
                        else:
            ...
            if flag and flag_panzhuang:
                if isinstance(res_item.origin_desc, list):
                    for item in res_item.origin_desc:
                        origin_label.append(res_item.origin_buwei + res_item.origin_zibuwei + item)
                        norm_label.append(res_item.norm_buwei + res_item.norm_zibuwei + item)

        else:
            # print("buwei  " + str(res_item.origin_buwei) + "zibuwei" + str(res_item.origin_zibuwei) +
            #       "desc  " +str(res_item.origin_desc),
            #       "origin:" + str(res_item.origin_sentence))
    ...
    def parse_sentence(self, sentence: str) -> ResultItem:
        ...
        # 单独处理
        sentence = sentence.replace("少许积液", "少量积液")
        sentence = sentence.replace("部分水肿", "局部水肿")
        sentence = sentence.replace("部分损伤", "局部损伤")
        sentence = sentence.replace("符合", "")
        sentence = sentence.replace("少量关节积液", "关节少量积液")
        sentence = sentence.replace("轻度髌股关节病", "髌股关节轻度骨关节病")
        sentence = sentence.replace("髌股关节病", "髌股关节骨关节病")
        ...
        list_level_key = self.list_level_key
        ...
        # 程度
        remove_subpart = self.remove_substring(remove_part, sub_part)
        level = self.find_longest_keyword(remove_subpart, list_level_key)
        # 剩下的就是描述
        remove_part_level = self.remove_substring_remain(remove_subpart, level)
        desc = self.remove_substring_remain(remove_part_level, level)
        ...
        if result_item.norm_zibuwei and self.dict_zibuwei_keepor[result_item.norm_zibuwei] == "否":
            ...
            result_item.upword_zibuwei = result_item.norm_zibuwei
        ...
        if desc in self.describe_alias_map:
            ...
            result_item.origin_desc = self.similar_words(sentence,desc,self.list_describe_key,True)
            if result_item.origin_desc:
                result_item.norm_desc = self.describe_alias_map[result_item.origin_desc]

        # 如果描述是单独句
        # 色素沉着绒毛结节性滑膜炎匹配到部位滑膜、髌股关节病、二分髌骨
        # 排除列表
        list_exclude = ['髌股关节', '髌骨', '滑膜']
        for alone_item in alone_sentence_list:
            if result_item.norm_desc in list_exclude and alone_item in sentence:
                result_item = ResultItem()
                result_item.origin_buwei = '膝关节'
                result_item.norm_buwei = '膝关节'
                result_item.origin_zibuwei = ''
                result_item.norm_zibuwei = ''
                result_item.origin_desc = sentence

                if alone_item in self.alone_sentence_alias_map:
                    result_item.norm_desc = self.alone_sentence_alias_map[alone_item]
                else:
                    result_item.norm_desc = alone_item

                if "多发" in result_item.norm_desc:
                    result_item.origin_desc = result_item.norm_desc.replace("多发", "")
                    result_item.norm_desc = result_item.norm_desc.replace("多发", "")
                    result_item.origin_level = "多发"
                    result_item.normal_level = "多发"

                result_item.origin_sentence = sentence
                result_item.confidence = 1
                return result_item

        if result_item.norm_buwei == '' and desc in alone_sentence_list:
            result_item = ResultItem()
            result_item.origin_buwei = '膝关节'
            result_item.norm_buwei = '膝关节'
            result_item.origin_zibuwei = ''
            result_item.norm_zibuwei = ''
            result_item.origin_desc = desc
            result_item.norm_desc = self.alone_sentence_alias_map[desc]
        ...
        # 程度相关的处理
        result_item.origin_level = level
        if level in self.level_alias_map:
            result_item.normal_level = self.level_alias_map[level]
    ...
    # 模糊匹配
    def similar_words(self,sentence,word, list_words, one):
        if word:
    ...
    def post_process(self, res_object: ResultItem) -> ResultItem:
        ...
        if keyword in res_object.origin_buwei:
            ...
            if "内侧" in res_object.origin_buwei:
                res_object.origin_buwei = ["内侧盘状半月板", "内侧半月板"]
                res_object.norm_buwei = res_object.origin_buwei
                res_object.upword_buwei = res_object.origin_buwei
            elif "外侧" in res_object.origin_buwei:
                res_object.origin_buwei = ["外侧盘状半月板", "外侧半月板"]
                res_object.norm_buwei = res_object.origin_buwei
                res_object.upword_buwei = res_object.origin_buwei
            elif "内外" in res_object.origin_buwei:
                res_object.origin_buwei = ["内外侧盘状半月板", "内侧半月板", "外侧半月板"]
                res_object.norm_buwei = res_object.origin_buwei
                res_object.upword_buwei = res_object.origin_buwei
            else:
                ...
                res_object.norm_buwei = res_object.origin_buwei
                res_object.upword_buwei = res_object.origin_buwei
            if res_object.origin_desc == '':
                res_object.origin_buwei = [res_object.origin_buwei[0]]
                res_object.norm_buwei = res_object.origin_buwei
                res_object.upword_buwei = res_object.origin_buwei
            ...
            # 部分盘状半月板，不全、不完全盘状半月板
            pan_level = ["部分", "不全", "不完全"]
            for item in pan_level:
                if item in res_object.origin_sentence:
                    res_object.origin_level = '不完全'
                    res_object.normal_level = '不完全'
        ...
        for keyword in ["内外", "前后", "肌肉软组织"]:
            if keyword in res_object.origin_buwei:
                ...
                if keyword == "肌肉软组织":
                    ...
                    if isinstance(parts_up,list) and len(parts_up) == 2:
                        part_up_1, part_up_2 = parts_up[0], parts_up[1]
                        res_object.upword_buwei = [part_up_1 + keyword[0] + part_up_2, part_up_1 + keyword[1] + part_up_2]

        for keyword in ["变性撕裂","积液积血","撕裂移位"]:
            if keyword in res_object.norm_desc:
                res_object.desc_notchange = res_object.norm_desc
                if keyword == "变性撕裂":
                    res_object.origin_desc = ["变性", "撕裂"]
                    res_object.norm_desc = ["变性", "撕裂"]
                    res_object.upword_desc = ["变性", "撕裂"]
                elif keyword == "积液积血":
                    res_object.origin_desc = ["积液", "积血"]
                    res_object.norm_desc = ["积液", "积血"]
                    res_object.upword_desc = ["积液", "积血"]
                elif keyword == "撕裂移位":
                    res_object.origin_desc = ["撕裂", "移位"]
                    res_object.norm_desc = ["撕裂", "移位"]
                    res_object.upword_desc = ["撕裂", "移位"]



        res_object.normal_label = self.get_norm_label(res_object)
        res_object.upword_label = self.get_upword_label(res_object)

        # return res_object
        return res_object

    def obj_split(self,test_obj):
        def ensure_list(item):
            return [item] if not isinstance(item, list) else item

        result = []
        norm_buwei = ensure_list(test_obj.norm_buwei)
        norm_desc = ensure_list(test_obj.norm_desc)

        other_attrs = {k: v for k, v in test_obj.__dict__.items() if k not in ['norm_buwei', 'norm_desc']}

        if "盘状" in norm_buwei[0]:
            res_temp = ResultItem(norm_buwei = norm_buwei, norm_desc = norm_desc, **other_attrs)
            res_temp.norm_buwei = norm_buwei[0].replace("盘状", "")
            res_temp.norm_desc = "盘状"
            result.append(res_temp)
            for buwei in norm_buwei[1:]:
                for desc in norm_desc:
                    res_temp_2 = ResultItem(norm_buwei = buwei, norm_desc = desc, **other_attrs)
                    res_temp_2.normal_level = self.find_longest_keyword(test_obj.origin_sentence.split("半月板")[1],
                                                                        self.list_level_key)
                    result.append(res_temp_2)
            return result


        for buwei in norm_buwei:
            for desc in norm_desc:
                result.append(ResultItem(norm_buwei = buwei, norm_desc = desc, **other_attrs))
        return result
    ...
    def preprocess_sentence(self, sentence: str) -> str:
        ...
        if re.search(self.keywords, sentence):
            return ''
        sentence = sentence.replace('变性、损伤', '变性')
        ...
        if sentence.startswith("多发"):
            sentence = "膝关节" + sentence
    ...
    def match_score_subpart(self,res_object : ResultItem):
        if res_object.origin_desc in alone_sentence_list:
            return 1
        ...
        # 保存原始描述
        if res_object.desc_notchange != '':
            origin_desc = res_object.desc_notchange
        else:
            origin_desc = res_object.origin_desc

        # 程度
        origin_level = res_object.origin_level

        origin_part_length = len(origin_part)
        origin_subpart_length = len(origin_subpart)
        origin_level_length = len(origin_level)

        if "符合" in  res_object.origin_sentence:
            sentence = res_object.origin_sentence.replace("符合", "")
            # print("符合：",res_object.origin_sentence,"   sentence:",sentence)
        else:
            sentence = res_object.origin_sentence

        # 去掉子部位后就是描述,重复计算了一遍描述
        remove_part = self.remove_substring(sentence, origin_part)
        remove_subpart = self.remove_substring_remain(remove_part, origin_subpart)
        desc = self.remove_substring_remain(remove_subpart, origin_level)
        ...
        if sentence:
            ...
            if desc in self.describe_alias_map.keys():
                res_score = round(
                    (origin_part_length + origin_subpart_length + origin_level_length + desc_length) / sentence_length, 3)
            else:
                if len(diagnosis_split) > 1:
                    ...
                    res_score = round(
                        (origin_part_length + origin_subpart_length + origin_level_length + desc_length * round(
